...
{
    public function index(Request $request,Raport $raport)
    {
        if ($request->ajax()) {
            // $keywords = $request->keywords;
            $collection = Raport::where('siswa_id', Auth::user()->id)->paginate(10);
            return view('page.siswa.raport.list', compact('collection'));
        }
        $raport = Raport::where('siswa_id', Auth::user()->id)->paginate(10);
        return view('page.siswa.raport.main',compact('raport'));
    }
    public function create()
    {
        ...
        return view('page.siswa.raport.input', ['task' => new Task, 'course' => $course, 'room'=> $room,'user'=>$user]);
    }
    ...
    public function edit()
    {
        ...
        $room = Room::get();
        $user = User::where('role','s')->get();
        return view('page.siswa.raport.list_detail', ['task' => new Task, 'course' => $course, 'room'=> $room,'user'=>$user,'raport'=>$raport]);
    }
    ...
    {
        $raport = Raport::where('siswa_id', Auth::user()->id)->get();
        $data = ['title' => 'Raport', 'raport'=>$raport];
        $pdf = PDF::loadView('page.siswa.raport.pdf', $data);
